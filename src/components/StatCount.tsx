...
export default function StatCount() {
  ...
  return (
    <section className="stat-count bg-secondary ~pb-[30px]/[60px] ~pt-[50px]/[80px]">
      <div className="stat-count__container container container--custom">
        <div className="state-count__content">
          
          <div className="flex flex-col md:flex-row items-center justify-between gap-[40px] md:~gap-[20px]/[60px]">
            ...
            {cargoData.map((item) => (
              <div key={item.id} className="bg-white ~rounded-[24px]/[30px] w-full flex flex-col gap-1 justify-center text-center border-[3px] border-primary group">
                <div className="relative">
                  <Icon icon={item.icon} className="inline-flex p-[14px] items-center justify-center bg-primary text-white fill-white rounded-full ~w-[50px]/[80px] ~h-[50px]/[80px] aspect-[1/1] absolute ~top-[-25px]/[-40px] left-1/2 -translate-x-1/2 group-hover:top-[-30px] group-hover:md:top-[-50px] animated" />
                </div>
